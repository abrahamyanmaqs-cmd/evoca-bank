...
export default function DibraryBox() {
  ...
  const defaultData = {
    title: "Dibrary - Digital Library",
    description: "Evocabank-ի նոր և գերժամանակակից «Թումանյան» մասնաճյուղում՝ Թումանյան 15 հասցեով, բացի ստանդարտ բանկային ծառայություններից, կգործի նաև Dibrary – digital library:",
    mainImage: "https://www.evoca.am/images-cache/menu/1/16117479662777/780x585.jpg",
    paragraph1: "#dibrary-ում կարելի է անվճար օգտվել էլեկտրոնային բիզնես ռեսուրսներից՝ e-Duke Journals Scholarly Collection, ASTM Compass, Edward Elgar Publishing Journals, Mathematical Sciences Publishers Journals, Royal Society Journals Collection, SAGE Premier և այլն․․․",
    paragraph2: "#dibrary-ի գրքերը, հոդվածները և հրապարակումները շարունակաբար համալրվելու են՝ ներառելով նաև համաշխարհային լավագույն համալսարանների փակ վճարովի ռեսուրսները։ Բացի էլեկտրոնային նյութերից, #dibrary-ում կարելի է գտնել նաև դասական գրքեր, ինչպես նաև ժամանակակից բիզնես գրականության բեսթսելլերները, որոնք կարելի է կարդալ տեղում, իսկ ցանկության դեպքում՝ նաև տանել տուն։",
    paragraph3: "#dibrary-ի բացման գործում մեզ աջակցել են՝ DLAA-ը՝ Հայաստանի թվային գրադարանների ասոցիացիան (Digital Library Association of Armenia), Ավետիք Իսահակյանի անվան գրադարանը և American Corner Yerevan-ը։ Նրանց աջակցության շնորհիվ՝ #dibrary-ում կարելի է անվճար օգտվել այնպիսի հրատարակիչների նյութերից, որոնց անդամակցությունը բավականին թանկ է և շատերի համար՝ անհասանելի։",
    logos: [
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "dibrary", "main_page");
  ...
  // Օժանդակ ֆունկցիա՝ տեքստում #dibrary բառը գունավորելու համար
  const renderFormattedText = (text) => {
    const parts = text.split('#dibrary');
    return parts.map((part, index) => (
      <React.Fragment key={index}>
        {part}
        {index < parts.length - 1 && (
          <span className="text-[#5E1EEB] font-semibold">#dibrary</span>
        )}
      </React.Fragment>
    ));
  };
    ...
    <section className="w-full bg-white py-16 px-4 md:px-12 font-sans">
      <div className="max-w-[1300px] mx-auto">
        ...
        <div className="flex flex-col gap-6 max-w-[1200px] mb-16">
          <p className="text-[#555555] text-[16px] md:text-[18px] leading-relaxed">
            {renderFormattedText(data.paragraph1)}
          </p>

          <p className="text-[#555555] text-[16px] md:text-[18px] leading-relaxed">
            {renderFormattedText(data.paragraph2)}
          </p>

          <p className="text-[#555555] text-[16px] md:text-[18px] leading-relaxed">
            {renderFormattedText(data.paragraph3)}
          </p>
        </div>
