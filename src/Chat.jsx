...
export default function Chat() {
        ...
        try {
          ...
          unsubscribeUsers = onSnapshot(collection(db, "users"), (snapshot) => {
            ...
            snapshot.forEach((docItem) => {
              if (docItem.id !== currentUser.uid) list.push(docItem.data());
            });
            setUsersList(list);
          });
        } catch (err) {
  ...
  return (
    <>
      {/* ԼՈՂԱՑՈՂ ՀԱՂՈՐԴԱԳՐՈՒԹՅԱՆ ԿՈՃԱԿ */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed bottom-5 right-5 z-50 bg-[#007bff] hover:bg-blue-600 text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center text-2xl transition transform hover:scale-105"
          title="Բացել չատը"
        >
          💬
        </button>
      ...
      {/* ԵԹԵ ՉԱՏԸ ԲԱՑՎԱԾ Է */}
      {isOpen && (
        <div className="fixed bottom-3 right-3 left-3 sm:left-auto sm:bottom-5 sm:right-5 z-50 bg-white border border-[#e4e6eb] rounded-[16px] shadow-2xl w-auto sm:w-[480px] h-[82vh] sm:h-[500px] lg:w-[550px] lg:h-[550px] overflow-hidden flex flex-col font-sans text-black">
          
          {/* ՎԵՐԵՎԻ ՀԱՏՎԱԾ / ՓԱԿԵԼՈՒ ԿՈՃԱԿ */}
          <div className="bg-gray-100 border-b p-2 flex items-center justify-between flex-shrink-0">
            <span className="text-xs font-bold text-gray-700">Evocabank Չատ</span>
            <button 
              onClick={() => setIsOpen(false)} 
              className="text-gray-500 hover:text-black font-bold text-lg px-2"
            >
              ✕
            </button>
          </div>
          ...
          {!user ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              <h3 className="text-lg font-bold mb-2">Մուտք գործեք չատ</h3>
              <p className="text-xs text-gray-500 mb-4">Շփվեք օգտատերերի հետ իրական ժամանակում:</p>
              <button onClick={handleLogin} className="bg-[#007bff] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-600 transition">
                Մուտք Google-ով
              </button>
            </div>
          ) : (
              ...
              {callState !== "idle" && (
                <div className="absolute inset-0 bg-black/95 z-50 flex flex-col items-center justify-center text-white p-4">
                  ...
                  {callState !== "rejected" && callState !== "no_answer" && (
                    <>
                      {callType === "video" && (
                        <div className="relative w-full flex-1 flex items-center justify-center bg-gray-900 rounded-lg overflow-hidden max-h-[300px] mb-4">
                  ...
                  <div className="flex gap-4 mt-4">
                    {isIncomingCall && callState === "ringing" ? (
                      <>
                        <button onClick={answerCall} className="bg-green-500 text-white px-4 py-2 rounded-full text-xs font-bold">Պատասխանել</button>
                        <button onClick={rejectCall} className="bg-red-500 text-white px-4 py-2 rounded-full text-xs font-bold">Մերժել</button>
                      </>
                    ) : (
                      <button onClick={endCall} className="bg-red-500 text-white px-4 py-2 rounded-full text-xs font-bold">Ավարտել զանգը</button>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className={`w-full sm:w-[180px] lg:w-[200px] border-r border-gray-200 flex flex-col justify-between bg-gray-50/50 ${selectedUser ? 'hidden sm:flex' : 'flex'} h-full`}>
                ...
                <div className="p-2 border-t bg-white flex items-center justify-between gap-1 flex-shrink-0">
                  <div className="flex items-center gap-1 overflow-hidden">
                    ...
                    <span className="text-[11px] font-semibold truncate max-w-[80px]">{user.displayName}</span>
                  </div>
