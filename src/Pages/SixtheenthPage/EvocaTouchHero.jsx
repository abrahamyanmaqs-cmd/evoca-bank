...
const EvocaTouchHero = () => {
  ...
  const initialData = {
    ...
    featuresIntro: "Նոր հավելվածն առաջարկում է գործառույթների լայն շրջանակ, որը ներառում է ամենօրյա բանկային գործընթացները՝ վերացնելով ֆիզիկական մասնաճյուղ այցելելու անհրաժեշտությունը: Առանց գրանցման և սպասարկման վճարի՝ օգտատերերը կարող են․",
    featuresList: [
  ...
  useEffect(() => {
    const fetchContent = async () => {
      const docRef = doc(db, "app_content", "evocatouch_page");
    ...
    <div className="w-full font-['MontserratARM',sans-serif] bg-white py-16 px-6 md:px-16 text-[#333333]">
      <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-5 text-[16px] md:text-[18px] text-[#333333] leading-relaxed">
          {content.paragraphs.map((text, i) => <p key={i}>{text}</p>)}
        </div>
        ...
        <div className="flex flex-col gap-5 text-[16px] md:text-[18px] text-[#333333] leading-relaxed pt-4">
          {content.closingParagraphs.map((text, i) => (
            <p key={i} className={i === content.closingParagraphs.length - 1 ? "text-[18px] md:text-[20px] pt-2" : ""}>
              {text}
            </p>
          ))}
        </div>
